...
如下代码是 RSA 签名与验证的过程，可以看到一些步骤是和 RSA 加解密一样的。随后进行签名与验证的库函数调用。
...
ECDSA 首先要对消息摘要进行结算，随后选择椭圆曲线来生成密钥对。接下来进行签名步骤，签名结果中重要的参数为 r 和 s。最后对使用公钥对签名进行验证，可以看到验证函数需要依次输入拖延曲线结构体、消息、消息长度、公钥 ctx.Q、以及签名结果 r 和 s。
