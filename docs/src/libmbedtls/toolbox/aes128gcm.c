...
/*
    ...
    ADD = 24825602bd12a984e0092d3e448eda5f
...
static uint8_t pt[] = {0xc3, 0xb3, 0xc4, 0x1f, 0x11, 0x3a, 0x31, 0xb7,
                       0x3d, 0x9a, 0x5c, 0xd4, 0x32, 0x10, 0x30, 0x69};

static uint8_t add[] = {0x24, 0x82, 0x56, 0x02, 0xbd, 0x12, 0xa9, 0x84,
                        0xe0, 0x09, 0x2d, 0x3e, 0x44, 0x8e, 0xda, 0x5f};

static uint8_t ct[] = {0x93, 0xfe, 0x7d, 0x9e, 0x9b, 0xfd, 0x10, 0x34,
                       0x8a, 0x56, 0x06, 0xe5, 0xca, 0xfa, 0x73, 0x54};

static uint8_t tag[] = {0x00, 0x32, 0xa1, 0xdc, 0x85, 0xf1, 0xc9, 0x78,
...
int main(void) {
  ...
  ret = mbedtls_cipher_auth_encrypt(&ctx, iv, sizeof(iv), add, sizeof(add), pt,
                                    sizeof(pt), buf, &len, tag_buf, 16);
  ...
  ret = mbedtls_cipher_auth_decrypt(&ctx, iv, sizeof(iv), add, sizeof(add), ct,
                                    sizeof(ct), buf, &len, tag, 16);
  assert_exit(ret == 0, ret);
